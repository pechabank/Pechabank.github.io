...
const generateFinancialAdvice = async (query, contextData) => {
  if (!ai) return "API Key configuration required to access PechaAI.";
  try {
    ...
    const response = await model.generateContent({
      ...
      config: {
        systemInstruction: "You are PechaAI, a cool, minimalist financial assistant for PechaBank. Your tone is professional but relaxed, slightly edgy. Keep answers concise, helpful, and formatted for easy reading.",
      }
    });
...
const GlassCard = ({ children, className = '', onClick, noAnimation = false, style }) => {
  return (
    <div 
      onClick={onClick}
...
const Card3D = ({ colorClass, title, icon, design, onClick, holderName, system = 'visa', className = "" }) => {
  return (
    <div 
      onClick={onClick} 
...
const PechaAssistant = ({ contextData }) => {
...
const FormInput = ({ value, onChange, placeholder, className = "", delay = "0ms" }) => (
  <input 
    type="text" 
...
const FormSection = ({ data, onChange, delayStart = 0 }) => {
  return (
    <div className="space-y-4">
       ...
       <FormInput value={data.name} onChange={(v) => onChange({...data, name: v})} placeholder="Имя Фамилия" delay={`${delayStart + 100}ms`} />
       <FormInput value={data.address} onChange={(v) => onChange({...data, address: v})} placeholder="Адрес доставки" delay={`${delayStart + 200}ms`} />
       <div className="grid grid-cols-2 gap-4">
          <FormInput value={data.phone} onChange={(v) => onChange({...data, phone: v})} placeholder="Телефон" delay={`${delayStart + 300}ms`} />
          <FormInput value={data.cvv} onChange={(v) => onChange({...data, cvv: v})} placeholder="Свой CVV" delay={`${delayStart + 400}ms`} />
       </div>
    </div>
  );
}
...
function App() {
  ...
  const [childTab, setChildTab] = useState('character');
          ...
          <section id="cards-section" className="min-h-screen py-20 px-4 md:px-8 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 md:gap-12">
              <div className="flex flex-col gap-6 group animate-slide-up-fade opacity-0" style={{ animationDelay: '200ms' }}>
                <Card3D colorClass={COLORS[0].class} title="Debit Core" onClick={() => setView('flow-debit')} />
                <div className="text-center group-hover:translate-y-[-5px] transition-transform duration-1000">
                  ...
                  <button onClick={() => setView('flow-debit')} className="w-full py-4 rounded-2xl glass-button text-white font-medium hover:bg-violet-600/20 hover:border-violet-500/50 active:scale-95">Оформить</button>
                </div>
              </div>
              <div className="flex flex-col gap-6 group animate-slide-up-fade opacity-0" style={{ animationDelay: '400ms' }}>
                <Card3D colorClass={COLORS[3].class} title="Credit Flow" onClick={() => setView('flow-credit')} />
                <div className="text-center group-hover:translate-y-[-5px] transition-transform duration-1000">
                  ...
                  <button onClick={() => setView('flow-credit')} className="w-full py-4 rounded-2xl glass-button text-white font-medium hover:bg-violet-600/20 hover:border-violet-500/50 active:scale-95">Оформить</button>
                </div>
              </div>
              <div className="flex flex-col gap-6 group animate-slide-up-fade opacity-0" style={{ animationDelay: '600ms' }}>
                <Card3D colorClass={MOCK_CHILD_CHARACTERS[0].colorClass} title="Junior" icon={MOCK_CHILD_CHARACTERS[0].emoji} onClick={() => setView('flow-child')} />
                <div className="text-center group-hover:translate-y-[-5px] transition-transform duration-1000">
                  <h3 className="text-2xl font-bold mb-2">Детская</h3>
                  <p className="text-white/50 text-sm mb-4">Для маленьких гениев.</p>
                  <button onClick={() => setView('flow-child')} className="w-full py-4 rounded-2xl glass-button text-white font-medium hover:bg-violet-600/20 hover:border-violet-500/50 active:scale-95">Оформить</button>
                </div>
              </div>
            </div>
          </section>
        ...
        <div className="relative z-20 min-h-screen pt-24 pb-12 px-4 max-w-5xl mx-auto" key="flow-debit">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-start">
            <div className="sticky top-24 animate-scale-in opacity-0" style={{ animationDelay: '100ms' }}>
              ...
              <Card3D colorClass={debitColor.class} title="Debit Core" holderName={formData.name} system={formData.system} />
            </div>
        ...
        <div className="relative z-20 min-h-screen pt-24 pb-12 px-4 max-w-5xl mx-auto" key="flow-credit">
          ...
          <div className="grid md:grid-cols-2 gap-12 items-start">
             <div className="sticky top-24 order-1 md:order-1 animate-scale-in opacity-0" style={{ animationDelay: '200ms' }}>
              ...
              <Card3D colorClass={creditColor.class} title="Credit Flow" holderName={formData.name} system={formData.system} />
        ...
        <div className="relative z-20 min-h-screen pt-24 pb-12 px-4 max-w-5xl mx-auto" key="flow-child">
          ...
          <div className="flex flex-col items-center mb-10 animate-slide-up-fade opacity-0">
            ...
            <div className="p-1 rounded-full bg-white/10 backdrop-blur-md flex shadow-lg">
              <button onClick={() => setChildTab('character')} className={`px-8 py-3 rounded-full text-sm font-bold transition-all duration-700 ${childTab === 'character' ? 'bg-violet-600 text-white shadow-lg scale-105' : 'text-white/60 hover:text-white'}`}>3D Персонажи</button>
              <button onClick={() => setChildTab('unique')} className={`px-8 py-3 rounded-full text-sm font-bold transition-all duration-700 ${childTab === 'unique' ? 'bg-pink-600 text-white shadow-lg scale-105' : 'text-white/60 hover:text-white'}`}>Уникальный дизайн</button>
            </div>
          </div>
          <div className="grid md:grid-cols-2 gap-12 items-start">
            <div className="sticky top-24 flex flex-col items-center animate-scale-in opacity-0" style={{ animationDelay: '200ms' }}>
               <div className="w-full max-w-md">
                 {childTab === 'character' ? (
                   <Card3D colorClass={childColor.class} title="Junior" icon={selectedCharacter.emoji} holderName={formData.name} system={formData.system} />
                 ) : (
                   <Card3D colorClass={selectedUnique.bg} title="Junior Unique" design={selectedUnique.id} holderName={formData.name} system={formData.system} />
                 )}
               </div>
